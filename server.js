...
function saveAll() {
    db.saveUsers(users).catch(e => console.error('saveUsers error:', e));
    db.saveKeys(keys).catch(e => console.error('saveKeys error:', e));
    db.saveSessions(sessions).catch(e => console.error('saveSessions error:', e));
}
...
function logActivity(username, action, details) {
    const entry = { username, action, details, timestamp: new Date().toISOString() };
    activity.push(entry);
    if (activity.length > 1000) activity = activity.slice(-1000);
    db.addActivityEntry(entry);
    saveAll();
}
...
function loadIrc() { /* moved to start() */ }
function saveIrc() {
    db.saveIrc(ircMessages.slice(-200)).catch(e => console.error('saveIrc error:', e));
}
...
app.post('/api/irc/send', (req, res) => {
    ...
    const user = users[username];
    if (!user || user.banned) return res.status(403).json({ success: false, error: 'Insufficient role' });
    const validRoles = ['user', 'admin', 'superadmin'];
    if (!validRoles.includes(user.role)) return res.status(403).json({ success: false, error: 'Insufficient role' });
    const hasActiveKey = Object.values(keys).some(k =>
        k.claimedBy === username && k.active !== false && new Date(k.expiresAt) > new Date()
    );
    if (!hasActiveKey) return res.status(403).json({ success: false, error: 'No active key' });
...
async function start() {
    ...
    ircMessages = data.ircMessages;
    ircIdCounter = data.ircIdCounter;
    initSuperAdmin();
    app.listen(PORT, () => {
